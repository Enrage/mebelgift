$(document).ready(function() {
	...
	$('.buy').click(function() {
		...
		price = parseFloat($(this).prev().children('span').html()); //получаем цену товара и преобразуем значение в число parseInt
		img = $(this).parent().children('a').find('img').attr('src'); //получаем ссылку на изображение, что бы отразить в корзине
		if (!img) img = $(this).parent().parent().children('.product_images').find('img').attr('src');
		title = $(this).parent().children('h3').find('a').html(); //название товара
		if (!title) title = $(this).parent().children('h2').text();
		//теперь нужно узнать есть ли в куках уже такой товар
		brand = $(this).parent().children('a.products_img').find('img').attr('class');
		if (!brand) brand = $(this).parent().children('h2').attr('id');
		category = $(this).parent().children('h3').attr('class');
	...
	$('.total').bind("change keyup", function() {
		...
		price = $(this).parent().prev().find('span').html(); //получаем цену товара
